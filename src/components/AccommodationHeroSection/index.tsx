...
interface AccommodationHeroSectionProps {
  accommodationPage: AccommodationPageType
  showButtons?: boolean
}

function getHeroImage(page: AccommodationPageType): Media | number | null {
  if ('heroBackgroundImage' in page) {
    return page.heroBackgroundImage
  }
  if ('heroImage' in page) {
...
export function AccommodationHeroSection({
  ...
  const heroImage = getHeroImage(accommodationPage)
        ...
        <Wrapper>
          ...
          >
            <div className="max-w-[670px] space-y-4 lg:space-y-[13px]">
              ...
              >
                {accommodationPage.heroTitle}
              </motion.h1>

              <motion.div
                className={showButtons ? 'border-l border-white pl-2.5 lg:pl-[10px]' : ''}
                variants={itemVariants}
              >
                <p className="font-raleway text-wrap text-base font-semibold leading-[1.2] text-white/90 sm:text-xl lg:text-[20px] lg:leading-[1.2]">
                  {accommodationPage.heroDescription}
                </p>
              </motion.div>
